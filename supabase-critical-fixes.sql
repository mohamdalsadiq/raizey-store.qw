...
-- ═══════════════════════════════════════════════════════════════════
-- RAIZEY STORE — إصلاحات حرجة (الدفعة 1 — نسخة قديمة)
--
-- لماذا مُعطّل؟ نُسخ بالكامل في supabase-critical-fixes-3.sql، وتشغيله
-- اليوم يُنشئ نسخة أقدم من create_wallet_order بتوقيع مختلف. وبما أن
-- PostgREST يختار النسخة حسب المعاملات المُرسَلة، فإن وجود توقيع قديم
-- إلى جانب الجديد يخلق مسار تجاوز كامل: ينادي المهاجم الدالة بمعاملات
-- التوقيع القديم فيتخطى فحوصات الحظر والصيانة والخيار الفرعي، ويسبب
-- أيضاً خطأ 42725 (function is not unique) للمستخدمين العاديين.
--
...
BEGIN
  RAISE EXCEPTION
    'ملف مهجور: supabase-critical-fixes.sql — نُسخ في -3.sql. تشغيله يُعيد توقيعاً قديماً لـ create_wallet_order يتجاوز فحوصات الأمان';
END $$;
