...
-- ═══════════════════════════════════════════════════════════════════
-- RAIZEY STORE — إصلاحات حرجة (الدفعة 2 — نسخة قديمة)
--
-- لماذا مُعطّل؟ نُسخ بالكامل في supabase-critical-fixes-3.sql، ويعتمد
-- على أعمدة غير موجودة في القاعدة الفعلية (مثل coupons.usage_count
-- و products.price_sdg) فيفشل في منتصف التنفيذ ويترك القاعدة في حالة
-- نصف مُطبَّقة — وهي أخطر من عدم التشغيل، لأن السياسات القديمة تبقى
-- قائمة بينما تُحذف الجديدة.
--
...
BEGIN
  RAISE EXCEPTION
    'ملف مهجور: supabase-critical-fixes-2.sql — نُسخ في -3.sql ويفشل على المخطط الحالي. استخدم -3.sql ثم -4.sql';
END $$;
