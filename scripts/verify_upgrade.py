...
options_sql = (ROOT / 'supabase-SQL-خيارات-المنتج-المتداخلة.sql').read_text(encoding='utf-8')
...
check('homepage renders strict categories only', 'catalog-subcategories-grid' in index and 'category.html?category_id=' in index and 'createSubcategoryCard' in index and 'popularProducts' not in index)
check('homepage keeps category scoped subcategories', 'renderCategories(categories || [], subcategories || [], products || [], exchangeRate)' in index and 'sectionSubcategories' in index and 'product.html?id=' not in index)
...
check('product supports simple products and absolute pricing', 'normalizeOptionTree' in product and 'selectedFinalOption' in product and 'selected_option: final ?' in product and "price_sdg: selectedUsd() * exchangeRate" in product)
check('product supports conditional nested options', 'children' in product and 'childGroupFor' in product and 'optionsAreRequired' in product and 'hasRequiredGap' in product)
...
check('admin variants and nested options are opt-in', 'hasOptionsToggle' in products_admin and 'optionsRequiredToggle' in products_admin and 'collectOptionTree' in products_admin and 'option_tree: collectOptionTree()' in products_admin and 'options_required: optionsRequiredToggle.checked' in products_admin)
...
check('nested option migration validates absolute USD prices', 'options_required' in options_sql and 'option_tree' in options_sql and 'validate_product_option_tree' in options_sql and 'price_usd' in options_sql)
