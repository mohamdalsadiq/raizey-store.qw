...
legacy_receipt_intel = (ROOT / 'assets/js/receipt-intel.js').read_text(encoding='utf-8')
legacy_receipt_judge = (ROOT / 'assets/js/receipt-judge-core.js').read_text(encoding='utf-8')
legacy_receipt_api = (ROOT / 'api/verify-receipt.js').read_text(encoding='utf-8')
...
check('homepage fetches active sections and category cards', "from('store_sections')" in index and "from('categories')" in index and 'renderCatalog' in index and 'categoriesContainer' in index)
check('homepage renders catalog sections and game cards', 'catalog-sections' in index and 'game-card' in index and 'section-title' in index)
check('category page loads active category products', "from('categories')" in category and "from('products')" in category and "eq('category_id', categoryId)" in category and "eq('is_active', true)" in category)
check('category page supports category id mode', 'categoryId' in category and 'renderCategory' in category and "from('categories')" in category and "from('products')" in category)
check('category renders inline product options', 'option-grid' in category and 'renderOptions' in category and 'selectedInternalOption' in category)
check('category keeps dynamic price and CTA flow', 'buildCartItem' in category and 'addToCart' in category and 'confirmOrder' in category and 'checkout-v2.html' in category)
...
check('receipt pipeline uses legacy Vercel Gemini first', '/api/verify-receipt' in receipt_pipeline and 'serverVerify' in receipt_pipeline and 'ReceiptIntel' in receipt_pipeline and 'process-receipt' not in receipt_pipeline)
check('legacy receipt endpoint and judge exist', (ROOT / 'api/verify-receipt.js').exists() and 'GEMINI_API_KEY' in legacy_receipt_api and 'ReceiptJudgeCore' in legacy_receipt_api and 'buildContext' in legacy_receipt_judge and 'judge' in legacy_receipt_judge)
check('receipt client keeps browser OCR fallback', 'ReceiptIntel.analyze' in checkout and 'ReceiptIntel.analyze' in checkout_v2 and 'RaizeyReceiptPipeline.analyze' in wallet and 'tesseract.js' in checkout and 'tesseract.js' in checkout_v2 and 'tesseract.js' in wallet and 'receipt-intel.js' in checkout and 'receipt-intel.js' in checkout_v2 and 'receipt-intel.js' in wallet and 'process-receipt' not in checkout and 'process-receipt' not in checkout_v2 and 'process-receipt' not in wallet)
check('category checkout handoff remains available', 'checkout-v2.html' in category and ('checkout-v2.html' in (ROOT / 'cart.html').read_text(encoding='utf-8') or 'checkout-v2.html' in product))
check('legacy receipt endpoint is active in Vercel path', (ROOT / 'api/verify-receipt.js').exists() and 'verify-receipt.js' not in (ROOT / 'vercel.json').read_text(encoding='utf-8'))
check('smart receipt allows only JPG PNG WEBP', 'image/jpeg,image/jpg,image/png,image/webp' in checkout and 'image/jpeg,image/jpg,image/png,image/webp' in checkout_v2 and 'image/jpeg,image/jpg,image/png,image/webp' in wallet and "'image/jpeg', 'image/png', 'image/webp'" in legacy_receipt_api and 'image/heic' not in legacy_receipt_api and 'image/heif' not in legacy_receipt_api and 'image/heic' not in supabase_client and 'image/heif' not in supabase_client)
check('smart receipt documents 5MB client limit', '5MB' in checkout and '5MB' in checkout_v2 and '5MB' in wallet)
check('legacy Vercel endpoint validates image payload', 'imageBase64' in legacy_receipt_api and 'mimeType' in legacy_receipt_api and 'GEMINI_API_KEY' in legacy_receipt_api and 'unsupported_image_format' in legacy_receipt_api and 'softReview' in legacy_receipt_api)
check('legacy receipt browser engine preserves OCR and deterministic rules', 'Tesseract' in legacy_receipt_intel and 'buildContext' in legacy_receipt_intel and 'buildContext' in legacy_receipt_judge and 'judge' in legacy_receipt_judge)
check('edge migration is retained only as rollback documentation', 'enforce_edge_receipt_scan_claim' in receipt_migration and (ROOT / 'supabase-SQL-rollback-نقل-فحص-الإيصالات.sql').exists())
