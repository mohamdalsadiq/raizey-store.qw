...
check('product enforces required sub-options', 'الخيار الفرعي الإجباري' in product and 'required: true' in product and 'validateSelection' in product)
check('product details follow actions', '    <p class="product-checkout-hint"' in product and product.index('    <p class="product-checkout-hint"') < product.index('    ${product.instructions ?'))
...
check('admin option group editor', 'opt-group' in products_admin and 'required: true' in products_admin and 'الخيار الفرعي الإجباري' in products_admin)
